...
int main (int argc, char * argv[] )
{
    ...
    {
		time ( &rawtime );
		got_time = localtime ( &rawtime );
		printf ( "Current local time and date: %s", asctime (got_time) );
	}
	...
	printf("The client says %s\n", buf);
		...
		printf ( "Current local time and date: %s", asctime (send_time) );
		char buf[256];
        strftime(buf, sizeof buf, "%F %T", send_time);
	if ((nbytes = sendto(sockfd, buf, strlen(buf), 0, (struct sockaddr *)&their_addr, addrlen)) == -1)
	{
