...
int main (int argc, char * argv[] )
{
	...
	int nbytes;                           //number of bytes we receive in our messag
	int bytes_read; 
	int size;
	...
	while(1)
	{
		...
		else if((val=strncmp(string1, "get", 3)) == 0)
		{
			...
			if((nbytes = sendto(sockfd, (void *)&size, strlen(msg), 0, (struct sockaddr *)&their_addr, addrlen)) == -1)
				{
						perror("listner: sendto");
						exit(1);			
				}
			...
			while (1)
			{
				bytes_read = fread(msg, 1, sizeof(msg)-1, fp);
				printf("%d\n", bytes_read);
